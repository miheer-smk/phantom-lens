...
# ---------------------------------------------------------------------------
# DEFECT-008 - deterministic seeding.
# The extractor originally contained a single unseeded np.random.choice, which made
# feature extraction non-reproducible across processes. Seeds are now derived
# per-video from a global seed, so output does not depend on how work is scheduled.
# ---------------------------------------------------------------------------
DEFAULT_SEED = 42

def _video_seed(video_path, seed=DEFAULT_SEED):
    """Stable per-video seed. Uses blake2b, not hash(), because Python's hash is
    randomised per process (PYTHONHASHSEED) and would reintroduce the defect."""
    import hashlib
    h = hashlib.blake2b(str(video_path).encode("utf-8"), digest_size=8).digest()
    return (int.from_bytes(h, "big") ^ int(seed)) % (2**32)
...
def extract_noise_physics(frame_gray, face_mask):
    ...
    center_mask = np.zeros(mag.shape, dtype=np.uint8)  # opencv>=4.11 compat: circle needs non-float64 dst
...
def extract_temporal_noise_stability(frames_gray, face_masks, rng_seed=42):
    """T1: Temporal Noise Stability — 3 features."""
    n = len(frames_gray)
    ...
    if len(mask_coords) > 100:
        # DEFECT-008 fix: was an UNSEEDED np.random.choice, making the extractor
        # non-deterministic. The generator is now derived from an explicit seed so the
        # result is independent of process, thread and scheduling order.
        idx = _rng(rng_seed).choice(len(mask_coords), 100, replace=False)
        for ci in idx:
            y, x = mask_coords[ci]
...
def process_single_video(video_path, label, max_frames=300, seed=DEFAULT_SEED):
    ...
    try:
        ...
        # ============================================================
        t1 = extract_temporal_noise_stability(frames_gray, face_masks, rng_seed=_video_seed(video_path, seed))

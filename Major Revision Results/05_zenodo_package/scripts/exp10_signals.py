...
DR=os.environ.get("DATASETS_ROOT","data")  # parent dir of FaceForensics++/Celeb-DF-v2
CASES={
 "tp":(f"{DR}/FaceForensics++/manipulated_sequences/Deepfakes/c23/videos/739_865.mp4","TP Deepfakes (fake, P=0.9996)"),
 "tn":(f"{DR}/FaceForensics++/original_sequences/youtube/c23/videos/949.mp4","TN real (P=0.022)"),
 "fn":(f"{DR}/FaceForensics++/manipulated_sequences/Face2Face/c23/videos/128_896.mp4","FN Face2Face (fake missed, P=0.078)"),
 "fp":(f"{DR}/Celeb-DF-v2/YouTube-real/00111.mp4","FP CelebDF real (P=0.993 -> flagged fake)"),
}
def signals(vpath):
...
for tag,(vp,title) in CASES.items():
    ...
    ax[0].plot(mouth,color="#B2182B",lw=1.2); ax[0].set_title("mouth texture stability",fontsize=9); ax[0].set_ylim(-0.2,1.05); ax[0].set_xlabel("frame",fontsize=8); ax[0].set_ylabel("frame-to-frame corr",fontsize=8); ax[0].grid(alpha=0.3,lw=0.5)
    ax[1].plot(ear,color="#2166AC",lw=1.2); ax[1].set_title("blink (eye-aspect-ratio)",fontsize=9); ax[1].set_xlabel("frame",fontsize=8); ax[1].set_ylabel("EAR",fontsize=8); ax[1].grid(alpha=0.3,lw=0.5)

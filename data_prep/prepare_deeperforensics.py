...
REAL_DIR = "data/deeperforensics/source_videos"
FAKE_DIR = "data/deeperforensics/manipulated_videos"
...
real_videos = find_all_videos(REAL_DIR)
...
# Prepare args
real_args = [(v, os.path.join(OUTPUT_REAL, f"deeper_real_{i:05d}"), FRAMES_PER_VIDEO)
             for i, v in enumerate(real_videos)]
fake_args = [(v, os.path.join(OUTPUT_FAKE, f"deeper_fake_{i:05d}"), FRAMES_PER_VIDEO)
             for i, v in enumerate(fake_videos)]

print(f"\nExtracting real video frames with {NUM_WORKERS} workers...")
with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
    list(tqdm(executor.map(extract_frames_from_video, real_args), total=len(real_args)))

print(f"\nExtracting fake video frames with {NUM_WORKERS} workers...")
with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
    list(tqdm(executor.map(extract_frames_from_video, fake_args), total=len(fake_args)))

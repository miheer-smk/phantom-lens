"""
Frequency Checkerboard Diagnostic — Celeb-DF v2
"""
import os, sys, numpy as np, imageio.v3 as iio, cv2
sys.path.insert(0, '.')
...
for vf in sorted(os.listdir(REAL_DIR))[:N_VIDEOS]:
    ...
    feat = np.mean([compute_freq_checkerboard(f) for f in frames], axis=0)
    real_feats.append(feat)
    print(f"  {vf}  per={feat[0]:.4f}  reg={feat[1]:.4f}  kl={feat[2]:.4f}")
...
print("RESULTS")
print(f"{'='*65}")
print(f"  {'Feature':<20} {'Real':>8} {'Fake':>8} {'Diff':>8} {'d':>8} {'Signal?':>10}")
print(f"  {'-'*60}")
...
for j, name in enumerate(names):
    ...
    print(f"  {name:<20} {rm:>8.4f} {fm:>8.4f} {rm-fm:>+8.4f} {d:>+8.4f} {sig:>10}  {direction}")
